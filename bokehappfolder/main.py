...
import datashader
import numpy as np
from bokeh.models import ColumnDataSource, CustomJS, Button, Select
import panel as pn
...
def modify_doc(doc):
    ...
    def onbuttonclick():
        invisText.js_on_change("value", CustomJS(code=jsupdateinfo, args=dict(xr=hvplot.state.x_range,
                                                         ...
                                                         data=ColumnDataSource(spectradf[spectradf['RT'].between(
    ...
    if len(sys.argv) > 2 and sys.argv[2] == "stoppable":
        layout = bokeh.layouts.layout([hvplot.state], [bt,invisText,invisText2])
    else:
        layout = bokeh.layouts.layout([hvplot.state],[bt,invisText,invisText2,stopbutton])
