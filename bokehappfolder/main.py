...
from pyopenms import MSExperiment, PeakMap, MzMLFile, PeakFileOptions, PeakSpectrum, DRange1, \
        ...
        def extractMetaData(f: Feature):
            ...
            if len(pep) != 0:
                ...
                if len(hits) != 0:
                    ...
                    # TODO what else
                    yield f.getUniqueId(), besthit.getSequence().toString(), f.getCharge(), f.getRT(), f.getMZ(), bb[0][
                        0], bb[1][0], bb[0][1], bb[1][1], f.getOverallQuality(), f.getIntensity()
                else:
                    yield f.getUniqueId(), None, f.getCharge(), f.getRT(), f.getMZ(), bb[0][0], bb[1][0], bb[0][1], \
                          bb[1][1], f.getOverallQuality(), f.getIntensity()
            else:
                yield f.getUniqueId(), None, f.getCharge(), f.getRT(), f.getMZ(), bb[0][0], bb[1][0], bb[0][1], bb[1][
                    1], f.getOverallQuality(), f.getIntensity()
...
def modify_doc(doc):
    ...
    if len(sys.argv) > 1:
        ...
        file = "C:/git/OpenMS/share/OpenMS/examples/FRACTIONS/BSA1_F1.mzML"
        # file = "/Volumes/Data/UPS1/mzML/UPS1_250amol_R1.mzML"
        # file = str(Path(__file__).resolve().parent) + "/static/data/BSA1.mzML"
        # fxmlfile = None
        fxmlfile = "C:/git/OpenMS/share/OpenMS/examples/FRACTIONS/BSA1_F1_idmapped.featureXML"
    ...
    invisText = Select(title="Option:", value="foo", options=["foo", "bar", "baz", "quux"], visible=False,
                       id="InvisibleText")
    invisText2 = Select(title="Option:", value="bar", options=["foo", "bar", "baz", "quux"], visible=False,
                        id="InvisibleText2")
    ...
    dynamic_col = bokeh.layouts.column(
        Div(text="Loading spectra...", width=300, height=150, css_classes=["lds-dual-ring"]), css_classes=["centered"])
    ...
    def init_2D_plot_and_update_btn():
        ...
        points = hv.Points(spectradf, kdims=['RT', 'mz'], vdims=['inty'], label="MS1 survey scans").options(
        ...
        if (fxmlfile is not None):
            ...
            featdf = fmap.get_df()
            rects = hv.Rectangles(featdf, kdims=["RTstart", "mzstart", "RTend", "mzend"]).options(color="value",
                                                                                                  alpha=0.1,
                                                                                                  tools=['tap'])
        else:
            rects = hv.Rectangles([])
        ...
        )  # weird.. I have no idea why one has to do this. But with one opts call you will get an error.
        # With a chained
        # .options(
        # you get a deprecation warning.
        raster = opts.apply_groups(raster,
            {
                'Image':
                {
                    'plot':
        ...
        def rect_annotation(index):
            if len(index) > 0:
                return hv.Table(featdf.iloc[index[0]].to_frame().T).options(title="Selected feature", height=70,
                                                                            width=800)
            else:
                return hv.Table([]).options(title="Selected feature", height=70, width=800)
        ...
        dyn = hv.DynamicMap(rect_selection, kdims=[], streams=[stream]).options(color="red", alpha=0.1)
